...
// TestAuth_ObservationsRequiresAuth tests that the observations endpoint
// requires auth when enabled.
func TestAuth_ObservationsRequiresAuth(t *testing.T) {
	app := NewTestApp(t, WithAuth("admin", "secret123"))
	defer app.Close()
...
func TestAuth_BasicAuth(t *testing.T) {
	...
	req, _ := http.NewRequest("GET", app.URL()+"/api/v1/observations", nil)
...
// TestAuth_TokenEndpointRejectsSSEToken verifies /auth/token only accepts
// Basic Auth and never an SSE token — otherwise a token could mint itself
// another token indefinitely.
func TestAuth_TokenEndpointRejectsSSEToken(t *testing.T) {
	app := NewTestApp(t, WithAuth("admin", "secret123"))
	defer app.Close()
	...
	if resp.StatusCode != http.StatusUnauthorized {
		t.Errorf("expected status 401 without Basic Auth, got %d", resp.StatusCode)
	}
}
